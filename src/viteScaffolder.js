...
/**
 * Creates main.js entry point that imports all code files
 * @param {string} srcDir - Source directory path
 * @param {Array<string>} codeFiles - Array of code file names
 * @param {Object} sketchInfo - Sketch metadata
 */
function createMainEntry(srcDir, codeFiles, sketchInfo) {
  const jsFiles = codeFiles.filter(file => file.endsWith('.js'));

  let mainContent = `/**
 * Main entry point for Vite
 * Auto-generated by opdl
 */
...
`;
  ...
  for (const jsFile of jsFiles) {
    mainContent += `import './${jsFile}';\n`;
  }

  // Import CSS files if any
  const cssFiles = codeFiles.filter(file => file.endsWith('.css'));
  for (const cssFile of cssFiles) {
    mainContent += `import './${cssFile}';\n`;
  }
